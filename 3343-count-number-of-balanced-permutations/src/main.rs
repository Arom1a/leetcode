...
pub fn count_balanced_permutations(num: String) -> i32 {
    ...
    dfs(
        0, 0, max_odd, &cnt, &psum, target, max_odd, &comb, &mut memo,
    ) as i32
}

fn main() {
    assert_eq!(count_balanced_permutations("123".to_string()), 2);
    assert_eq!(count_balanced_permutations("112".to_string()), 1);
    assert_eq!(count_balanced_permutations("12345".to_string()), 0);
    println!("All passed!");
}
